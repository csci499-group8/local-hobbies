...
export const hobbyKeys = {
    ...
    globalHobbies: () => [...hobbyKeys.all, 'globalHobbies'] as const,
...
export const useHobby = () => {
    ...
    const globalHobbiesQuery = useQuery({
        queryKey: hobbyKeys.globalHobbies(),
    ...
    const deleteHobbyPhotoMutation = useMutation({
        mutationFn: (photoId: string) => hobbyService.deleteHobbyPhoto(photoId),
        onSuccess: (_, photoId) => {
            queryClient.setQueryData(hobbyKeys.photos(), (prev: HobbyPhotoResponse[] | undefined) =>
    ...
    return {
        ...
        //global hobbies
        globalHobbies: globalHobbiesQuery.data ?? [],
        globalHobbiesLoading: globalHobbiesQuery.isLoading,
        globalHobbiesError: globalHobbiesQuery.error?.message ?? null,
