...
public interface OneTimeAvailabilityRepository extends JpaRepository<OneTimeAvailability, UUID> {
    ...
    @Query("""
        SELECT o FROM OneTimeAvailability o
        WHERE o.userId = :userId
        AND o.start > :cutoffDateTime
        ORDER BY o.start
    """)
    List<OneTimeAvailability> findActiveByUserId(@Param("userId") UUID userId,
                                                 @Param("cutoffDateTime") OffsetDateTime cutoffDateTime);
