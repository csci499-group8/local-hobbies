...
public class JsonNullableMapper {
     ...
     * Returns source value if present, otherwise current value. unwrap() is explicitly
     * called in @Mapper annotations because @Condition JsonNullableMapper.isPresent()
    ...
    /**
     * unwrap method for sources that must be transformed to another type. Returns
     * transformed source value if present, otherwise current value. unwrap() is explicitly
     * called in @Mapper annotations because @Condition JsonNullableMapper.isPresent()
     * does not work to prevent the field from being mapped if isPresent() = false.
     * Instead, if !isPresent(), an else { set field = null; } statement is generated.
     */
